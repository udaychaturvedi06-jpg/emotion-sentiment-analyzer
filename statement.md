...
Because of this, systems that depend on just one input often give incomplete or misleading results. For example, a person might look normal but sound stressed while speaking. This creates a gap between how machines understand emotions and how humans actually express them.

This project tries to reduce that gap by combining both facial expressions and voice input in one system. It captures the user’s face using a webcam and records their voice, then analyzes both to give a final emotional result.
...
It also includes combining both face and voice results to produce a final output, using simple gestures to trigger actions, displaying everything through a graphical interface, and maintaining a short history of recent results.
